...
// ----------------------------------------------------------------------------------
int PathFinding::CreatePath(iPoint& origin, iPoint& destination)
{
	...
	else
	{
		...
		while (open.list.count() > 0)
		{
			...
			ListItem<PathNode>* LowScoreCell = open.GetNodeLowestScore();

			PathNode* currentCell = &close.list.add(LowScoreCell->data)->data;
			open.list.del(LowScoreCell);
